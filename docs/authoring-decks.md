...
| `body` | `sans_family` | regular (`**bold**` inline) | **1.6** |
...
### Containers: grid or absolute EMU
...
Pass `check=False` to silence; or in CI, raise on a non-empty `fit_report`.
...
- **Key APIs**: `measure_text`, `FitResult`, `report_fit`, `fit_report`,
  `format_fit`, `overflows`, `save(check=…)`, `to_google_slides(check=…)`.
